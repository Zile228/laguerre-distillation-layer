...
class BUSIDataset(Dataset):
    ...
    def __init__(
        ...
        self.samples: List[Tuple[str, str]] = []  # (img_path, mask_path)
        ...
        for cls in classes:
            ...
            for img_path in imgs:
                # Find corresponding mask (may be <name>_mask.png or <stem>_mask.png)
                mask_path = img_path.with_name(
                    img_path.stem + "_mask" + img_path.suffix
                )
                if mask_path.exists():
        ...
        print(f"  BUSIDataset [{split:5s}]: {len(self.samples)} samples")
    ...
    def __getitem__(self, idx: int):
        ...
        # Binarise mask (some BUSI masks have values 0/255)
